...
import {IPM_Motif_Statut} from "../../Models/IPM_Motif_Statut";
...
export class ListeEmployesComponent implements OnInit /*,AfterViewInit*/ {
  ...
  motif:IPM_Motif_Statut=  new IPM_Motif_Statut();
 ...
 this.statuttt=false
  ...
  getEmployeByMatricule(){
    console.log(this.matricule);
      ...
      this.emp_service.getEmployeByMatricule(this.matricule).subscribe({
        next: (data) => {
          this.employeRech = data
          if(this.employeRech == null){
            //this.showNotification('top', 'center', 1, '<b>  Matricule  incorrecte !!!</b> :')
            this.showNotification('top', 'center', 3, "<b> Matricule  incorrecte !!!</b> :")
  ...
  getMotif(){
    ...
    //this.motifStatut=stu.libelleStatut
      this.emp_service.getMotif().subscribe(
